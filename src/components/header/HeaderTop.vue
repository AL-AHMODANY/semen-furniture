...
<template>
  <div
    class="bg-[#222] text-[#999] text-[11px]"
    style="font-family: &quot;Open Sans&quot;, Arial, sans-serif"
  >
    <div
      class="site-shell flex flex-wrap items-center justify-between gap-1 h-auto sm:h-[36px]"
    >
      ...
      <div class="flex flex-wrap items-center gap-1">
        ...
        <div class="relative" @click.stop>
          <button
            @click="
              ...
              currOpen = false;
            "
            class="flex items-center gap-1.5 px-2 sm:px-3 h-[36px] hover:text-[#c0392b] hover:bg-[#2e2e2e] transition-all border-r border-[#3a3a3a]"
          >
            <img
          ...
          <Transition name="dd">
            <ul
              v-if="langOpen"
              class="absolute top-full left-0 bg-[#2a2a2a] border border-[#3a3a3a] min-w-[160px] z-[9999] shadow-xl py-1"
            >
              <li
              ...
              >
                <img
                  :src="lang.flag"
                  :alt="lang.label"
                  class="w-[20px] h-[14px] object-cover rounded-[1px] shadow-sm flex-shrink-0"
                />
        ...
        <div class="relative" @click.stop>
          ...
          <Transition name="dd">
            <ul
              v-if="currOpen"
              class="absolute top-full left-0 bg-[#2a2a2a] border border-[#3a3a3a] min-w-[110px] z-[9999] shadow-xl py-1"
            >
              <li
      ...
      <div class="flex flex-wrap items-center gap-1">
        ...
        <button
          @click="
            authStore.state.isLoggedIn
              ? authStore.openAccount()
              : authStore.openLogin()
          "
          class="flex items-center gap-1.5 px-2 sm:px-3 h-[36px] hover:text-[#c0392b] hover:bg-[#2e2e2e] transition-all border-l border-[#3a3a3a]"
        >
          <svg
            class="w-3 h-3"
            fill="none"
          ...
          <span
            ...
            class="hidden sm:inline text-white font-semibold max-w-[90px] truncate"
          >
            {{ authStore.state.user?.name.split(" ")[0] }}
          </span>
          <span class="hidden sm:inline" v-else>My Account</span>
        </button>
        ...
        >
          ...
          <span
            v-if="shopStore.wishlistCount.value"
            class="hidden sm:inline text-white"
            >({{ shopStore.wishlistCount.value }})</span
          >
        </RouterLink>
